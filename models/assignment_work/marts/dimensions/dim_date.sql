...
WITH all_dates AS (
   ...
   SELECT-- Date dimension shared by both restaurant applications and 311 requests
...
SELECT *
FROM date_dimension DISTINCT CAST(time_of_submission AS DATE) AS full_date
   FROM {{ ref('stg_nyc_open_restaurant_apps') }}
   WHERE time_of_submission IS NOT NULL
),
